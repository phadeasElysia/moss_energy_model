...
to setup
  clear-all
  set gas-prices 2.00  ; Example price per gallon in dollars
  set oil-prices 1.50  ; Example price per gallon in dollars
  set traditional-heating-system-prices 3000  ; Example system price in dollars
  set cop 3.5  ; Example COP value
  set installation-capacity 100  ; Number of systems that can be installed per year
  set hassle-factor 5  ; Arbitrary scale from 1 to 10
  set discount-factor 0.1  ; 10% discount on heat pumps
  set scale-parameter 15
  set gas-boiler-price 3250
  set oil-boiler-price 3650
  set electric-boiler-price 2650
  set ASHP 10000
  set GSHP 15000
  set solid-fuel-price 5000

  create-households 10000 [
    setxy random-xcor random-ycor
    set shape "house"
    let rnd random-float 1  ; Generates a random number between 0 and 1
    if rnd > 0.95  [
      set heating-system-type "solid-fuel"  ; 50% probability
    ]
    if rnd > 0.91 and rnd <= 0.95[
      set heating-system-type one-of ["ASHP" "GSHP"]
    ]
    if rnd <= 0.91 and rnd >= 0.67 [
      set heating-system-type "oil"  ; Additional 30% probability
    ]
    if rnd < 0.67  [
      set heating-system-type "electric"  ; Remaining 20% probability
    ]
    set heating-system-age random 15
    ;set aware-of-heat-pumps one-of [true false]
    set heating-budget 5000 + random 1500
...
; Additional submodel procedures...
@#$#@#$#@
GRAPHICS-WINDOW
210
10
